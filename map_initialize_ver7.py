...
# 生成qpv.txt
def generate_qpv(qpv):
    qpv_map = open('Samples/qpv_map.txt', 'w')
    for scile_2d in qpv:
        np.savetxt(qpv_map, scile_2d, fmt = '%f', delimiter = ',')
    qpv_map.close()

# 生成map_i.csv, i=1,2,...,popSize
def generate_csv(maps,i):
    '''
    input: maps: ndarray,(25,20)
    i: scalar, means the ith samples of the pop
    '''
    with open('Samples/map_'+str(i)+'.csv', 'w', newline='') as map:
...
def getQpv(qpv):
    qpv_map = open('qpv_map.txt', 'w')  #保存为.txt文件
    for scile_2d in qpv:
        np.savetxt(qpv_map, scile_2d, fmt='%f', delimiter=',')
    qpv_map.close()
...
def main():
    ...
    df.to_csv("map.csv",index= False, header= False)     # ndarray, (180,9)
    getQpv(qpv)
